...
function goToDrawing() {
  pendingNext = nextQuestion;

  currentDrawWord =
    (current.activeVocab && current.activeVocab.word) ||
    current.kanji ||
    current.word ||
    "";
...
function finishDrawing() {
  ...
  async function showDrawingScreen() {
  const chars = [...currentDrawWord];
  ...
  app.innerHTML = `
    <div class="center">
      <div class="card">
        ...
        <h3>Draw</h3>
        <div class="word">${currentDrawWord}</div>

        <div class="draw-container">
          <div class="kanji-diagram" id="diagram"></div>
          <canvas id="drawCanvas" class="draw-canvas"></canvas>
        </div>

        <button onclick="clearCanvas()">Clear</button>
        <button onclick="finishDrawing()">Continue</button>
...
function showSimpleFeedback(isCorrect) {
  ...
  return !(fb.correctInputs || []).includes(normalized);
	...
	<div style="margin:10px 0;">
      ...
      <button onclick="goToDrawing()">Continue</button>
